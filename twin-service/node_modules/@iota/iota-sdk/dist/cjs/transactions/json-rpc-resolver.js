...
async function setGasPayment(transactionData, options) {
  if (!transactionData.gasConfig.payment) {
    ...
    transactionData.gasConfig.payment = paymentCoins.map(
      (payment) => (0, import_valibot.parse)(import_internal.ObjectRef, payment)
    );
  }
}
async function resolveObjectReferences(transactionData, options) {
  ...
  const dedupedIds = [
    ...new Set(
      objectsToResolve.map((input) => (0, import_utils.normalizeIotaObjectId)(input.UnresolvedObject.objectId))
    )
  ];
