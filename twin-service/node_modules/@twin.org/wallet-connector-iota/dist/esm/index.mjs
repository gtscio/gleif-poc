...
class IotaFaucetConnector {
    ...
    constructor(options) {
        ...
        Guards.string(this.CLASS_NAME, "options.config.endpoint", options.config.endpoint);
...
class IotaWalletConnector {
    ...
    async create(identity) {
        ...
        await this._vaultConnector.setSecret(Iota.buildMnemonicKey(identity, this._config.vaultMnemonicId), mnemonic);
        const seed = Bip39.mnemonicToSeed(mnemonic);
        await this._vaultConnector.setSecret(Iota.buildSeedKey(identity, this._config.vaultSeedId), Converter.bytesToBase64(seed));
    }
    /**
    ...
    async getAddresses(identity, accountIndex, startAddressIndex, count, isInternal) {
        ...
        return Iota.getAddresses(seed, this._config.coinType ?? Iota.DEFAULT_COIN_TYPE, accountIndex, startAddressIndex, count, isInternal);
    }
    /**
    ...
    async transfer(identity, addressSource, addressDest, amount) {
        ...
        try {
            const result = await Iota.prepareAndPostValueTransaction(this._config, this._vaultConnector, undefined, identity, this._client, addressSource, amount, addressDest);
