...
class IotaNftUtils {
    ...
    static nftIdToObjectId(nftIdUrn) {
        ...
        const nftUrn = core.Urn.fromValidString(nftIdUrn);
        const parts = nftUrn.parts();
        if (parts.length !== 5) {
            throw new core.GeneralError(IotaNftUtils.CLASS_NAME, "invalidNftIdFormat", {
                id: nftIdUrn
            });
        }
        return parts[4];
...
class IotaNftConnector {
    ...
    constructor(options) {
        ...
        this._walletConnector = walletModels.WalletConnectorFactory.get(options.walletConnectorType ?? "wallet");
        ...
        core.Guards.stringValue(this.CLASS_NAME, "this._contractName", this._contractName);
        this._gasBudget = this._config.gasBudget ?? 1_000_000_000;
        core.Guards.number(this.CLASS_NAME, "this._gasBudget", this._gasBudget);
        if (this._gasBudget <= 0) {
            throw new core.GeneralError(this.CLASS_NAME, "invalidGasBudget", { gasBudget: this._gasBudget });
        }
    ...
    async start(nodeIdentity, nodeLoggingConnectorType, componentState) {
        ...
        try {
            ...
            if (!contractData) {
                throw new core.GeneralError(this.CLASS_NAME, "contractDataNotFound", {
                    contractName: this._contractName
                });
            }
            // Convert base64 package(s) to bytes
            let compiledModules;
            if (core.Is.arrayValue(contractData.package)) {
                compiledModules = contractData.package.map((pkg) => Array.from(core.Converter.base64ToBytes(pkg)));
            }
            else {
                compiledModules = [Array.from(core.Converter.base64ToBytes(contractData.package))];
            }
            const contractDeployments = componentState?.contractDeployments ?? {};
            if (core.Is.stringValue(contractDeployments[contractData.packageId])) {
                ...
                const packageExists = await dltIota.Iota.packageExistsOnNetwork(this._client, contractDeployments[contractData.packageId]);
                if (packageExists) {
            ...
            const result = await dltIota.Iota.prepareAndPostTransaction(this._config, this._vaultConnector, nodeLogging, nodeIdentity, this._client, controllerAddress, txb, {
                dryRunLabel: this._config.enableCostLogging ? "deploy" : undefined
            });
            if (result.effects?.status?.status !== "success") {
                throw new core.GeneralError(this.CLASS_NAME, "deployTransactionFailed", {
            ...
            if (!core.Is.stringValue(deployedPackageId)) {
                throw new core.GeneralError(this.CLASS_NAME, "packageIdNotFound", {
    ...
    async mint(controllerIdentity, tag, immutableMetadata, metadata) {
        this.ensureStarted();
        core.Guards.stringValue(this.CLASS_NAME, "controllerIdentity", controllerIdentity);
        ...
        try {
            ...
            const result = await dltIota.Iota.prepareAndPostTransaction(this._config, this._vaultConnector, this._logging, controllerIdentity, this._client, address, txb, {
            ...
            if (!core.Is.stringValue(createdObjectId)) {
                throw new core.GeneralError(this.CLASS_NAME, "failedToGetNftId", undefined);
            }
            const urn = new core.Urn("nft", `${IotaNftConnector.NAMESPACE}:${this._config.network}:${this._deployedPackageId}:${createdObjectId}`);
            return urn.toString();
        }
        catch (error) {
            throw new core.GeneralError(this.CLASS_NAME, "mintingFailed", undefined, dltIota.Iota.extractPayloadError(error));
        }
    }
    /**
     ...
     */
    async resolve(nftId) {
        core.Guards.stringValue(this.CLASS_NAME, "nftId", nftId);
        try {
            ...
            if (!object.data?.content) {
                throw new core.GeneralError(this.CLASS_NAME, "nftNotFound", { nftId });
            }
            ...
            if (content.immutable_metadata) {
                try {
                    ...
                    throw new core.GeneralError(this.CLASS_NAME, "invalidImmutableMetadata", { nftId }, error);
                }
            }
            ...
            if (content.metadata) {
                try {
                    ...
                    throw new core.GeneralError(this.CLASS_NAME, "invalidMetadata", { nftId }, error);
                }
            }
            return {
            ...
            throw new core.GeneralError(this.CLASS_NAME, "resolvingFailed", undefined, dltIota.Iota.extractPayloadError(error));
        }
    }
    /**
     * Burn an NFT.
    ...
    async burn(controllerIdentity, id) {
        ...
        try {
            ...
            const result = await dltIota.Iota.prepareAndPostTransaction(this._config, this._vaultConnector, this._logging, controllerIdentity, this._client, ownerAddress, txb, {
                dryRunLabel: this._config.enableCostLogging ? "burn" : undefined
            });
            if (result.effects?.status?.status !== "success") {
                throw new core.GeneralError(this.CLASS_NAME, "burningFailed", {
                    error: result.effects?.status?.error
                });
            }
        }
        catch (error) {
            throw new core.GeneralError(this.CLASS_NAME, "burningFailed", undefined, dltIota.Iota.extractPayloadError(error));
        }
    }
    /**
    ...
    async transfer(controller, nftId, recipientIdentity, recipientAddress, metadata) {
        ...
        if (!core.Is.undefined(metadata)) {
            core.Guards.object(this.CLASS_NAME, "metadata", metadata);
        }
        const urnParsed = core.Urn.fromValidString(nftId);
        if (urnParsed.namespaceMethod() !== IotaNftConnector.NAMESPACE) {
            throw new core.GeneralError(this.CLASS_NAME, "namespaceMismatch", {
                namespace: IotaNftConnector.NAMESPACE,
                id: nftId
        ...
        try {
            ...
            if (currentNft.owner !== controller) {
                throw new core.GeneralError(this.CLASS_NAME, "transferFailed", {
            ...
            const result = await dltIota.Iota.prepareAndPostTransaction(this._config, this._vaultConnector, this._logging, controller, this._client, ownerAddress, txb, {
    ...
    async update(controllerIdentity, id, metadata) {
        ...
        try {
            ...
            if (result.effects?.status?.status !== "success") {
                throw new core.GeneralError(this.CLASS_NAME, "updateFailed", {
                    error: result.effects?.status?.error
                });
            }
        }
        catch (error) {
            throw new core.GeneralError(this.CLASS_NAME, "updateFailed", undefined, dltIota.Iota.extractPayloadError(error));
        }
    }
    /**
    ...
    ensureStarted() {
        if (!this._deployedPackageId) {
            throw new core.GeneralError(this.CLASS_NAME, "connectorNotStarted", {
     ...
     */
    getModuleName() {
        return core.StringHelper.snakeCase(this._contractName);
    }
    /**
    ...
    getOwnerAddress(nftId, object) {
        ...
        throw new core.GeneralError(this.CLASS_NAME, "nftOwnerNftFound", { nftId });
    }
}

exports.IotaNftConnector = IotaNftConnector;
exports.IotaNftUtils = IotaNftUtils;
