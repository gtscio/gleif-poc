import { Guards, StringHelper, BaseError, GeneralError, NotFoundError, AlreadyExistsError, Is, Converter, RandomHelper, ObjectHelper } from '@twin.org/core';
import { Ed25519 } from '@twin.org/crypto';
import { LoggingConnectorFactory } from '@twin.org/logging-models';
import { VaultKeyType, VaultEncryptionType } from '@twin.org/vault-models';
import { FetchHelper, HttpMethod, FetchError } from '@twin.org/web';
...
class HashicorpVaultConnector {
    ...
    constructor(options) {
        ...
        Guards.stringValue(this.CLASS_NAME, "options.config.token", options.config.token);
    ...
    async bootstrap(nodeLoggingConnectorType) {
        ...
        try {
            await FetchHelper.fetch(this.CLASS_NAME, `${this._baseUrl}/sys/health`, HttpMethod.GET, undefined, { headers: this._headers });
    ...
    async setSecret(name, data) {
        Guards.stringValue(this.CLASS_NAME, "name", name);
        Guards.defined(this.CLASS_NAME, "data", data);
        try {
    ...
    async getSecret(name) {
        ...
        try {
            const path = this.getSecretPath(name);
            const url = `${this._baseUrl}/${path}`;
            const response = await FetchHelper.fetchJson(this.CLASS_NAME, url, HttpMethod.GET, undefined, { headers: this._headers });
    ...
    async removeSecret(name) {
        ...
        try {
            ...
            const url = `${this._baseUrl}/${path}`;
            await FetchHelper.fetchJson(this.CLASS_NAME, url, HttpMethod.DELETE, undefined, { headers: this._headers });
        }
        catch (err) {
    ...
    async createKey(name, type) {
        Guards.stringValue(this.CLASS_NAME, "name", name);
        Guards.arrayOneOf(this.CLASS_NAME, "type", type, Object.values(VaultKeyType));
        ...
        try {
            ...
            await FetchHelper.fetchJson(this.CLASS_NAME, url, HttpMethod.POST, payload, { headers: this._headers });
    ...
    async addKey(name, type, privateKey, publicKey) {
        Guards.stringValue(this.CLASS_NAME, "name", name);
        Guards.arrayOneOf(this.CLASS_NAME, "type", type, Object.values(VaultKeyType));
        Guards.uint8Array(this.CLASS_NAME, "privateKey", privateKey);
        try {
            // Check if the key exists
            const existingVaultKey = await this.readKey(name);
            if (existingVaultKey) {
    ...
    async renameKey(name, newName) {
        Guards.stringValue(this.CLASS_NAME, "name", name);
        Guards.stringValue(this.CLASS_NAME, "newName", newName);
        try {
    ...
    async sign(name, data) {
        Guards.stringValue(this.CLASS_NAME, "name", name);
        Guards.uint8Array(this.CLASS_NAME, "data", data);
        try {
            await this.readKey(name);
        }
        catch (err) {
        ...
        try {
            ...
            const response = await FetchHelper.fetchJson(this.CLASS_NAME, url, HttpMethod.POST, payload, { headers: this._headers });
    ...
    async verify(name, data, signature) {
        Guards.stringValue(this.CLASS_NAME, "name", name);
        Guards.uint8Array(this.CLASS_NAME, "data", data);
        Guards.uint8Array(this.CLASS_NAME, "signature", signature);
        try {
            await this.readKey(name);
        }
        catch (err) {
    ...
    async encrypt(name, encryptionType, data) {
        Guards.stringValue(this.CLASS_NAME, "name", name);
        Guards.arrayOneOf(this.CLASS_NAME, "encryptionType", encryptionType, Object.values(VaultEncryptionType));
        Guards.uint8Array(this.CLASS_NAME, "data", data);
        let keyDetails;
        try {
            keyDetails = await this.readKey(name);
        ...
        try {
            if (encryptionType === VaultEncryptionType.ChaCha20Poly1305 &&
    ...
    async decrypt(name, encryptionType, encryptedData) {
        Guards.stringValue(this.CLASS_NAME, "name", name);
        Guards.arrayOneOf(this.CLASS_NAME, "encryptionType", encryptionType, Object.values(VaultEncryptionType));
        Guards.uint8Array(this.CLASS_NAME, "encryptedData", encryptedData);
        try {
            await this.readKey(name);
        }
        catch (err) {
    ...
    async restoreKey(name, backup) {
        Guards.stringValue(this.CLASS_NAME, "name", name);
        Guards.stringValue(this.CLASS_NAME, "backup", backup);
        ...
        try {
            ...
            if (response?.data) {
                ...
                if (keyType === VaultKeyType.Ed25519) {
                    key = Converter.base64ToBytes(keys[keyVersion]).slice(0, keyPath === "public-key" ? Ed25519.PUBLIC_KEY_SIZE : Ed25519.PRIVATE_KEY_SIZE);
                }
                else {
...
export { HashicorpVaultConnector };
